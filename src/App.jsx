...
function Header({ view, query, onSearch, onHome, onHowItWorks }) {
  const searchableView = view === 'results'
  return <header className={`site-header ${searchableView ? 'site-header--searching' : ''}`}>
    <button className="wordmark" onClick={onHome} aria-label="CineMood home">cine<span>mood</span></button>
    {searchableView && <div className="header-search"><MoodSearch key={query} compact initialValue={query} onSearch={onSearch} label="Search another feeling" /></div>}
...
function Opening({ onEnter }) {
  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => gsap.timeline({ defaults: { ease: 'power3.out' } })
      .from('.opening-mark span', { opacity: 0, y: 30, filter: 'blur(10px)', duration: 1.2, stagger: .09 })
      .from('.opening-tagline, .opening-cta', { opacity: 0, y: 14, duration: .7, stagger: .13 }, '-=.45'), introRef)
...
function SearchLanding({ onSearch }) {
  ...
  return <><main className="hero" ref={heroRef}><div className="hero-halo" aria-hidden="true" /><div className="hero-copy"><p className="eyebrow"><span className="live-dot" /> semantic film discovery</p><h1>Tell us how you<br /><em>want to feel.</em></h1><p className="hero-sub">CineMood understands meaning—not just keywords or genres—to find films that move at the same emotional frequency.</p></div><div className="hero-search-cluster"><MoodSearch onSearch={onSearch} /><div className="search-promise"><span><BrainCircuit size={15} /> MiniLM runs on this device</span><i /><span>no live movie API</span></div></div><div className="flow-proof" aria-label="How CineMood works"><span>describe a feeling</span><ArrowRight size={15} /><span>AI embeds the meaning</span><ArrowRight size={15} /><span>find your film</span></div></main><HowItWorks /></>
}

function AnalysisScreen({ phase, progress, query, error, onBack }) {
  const steps = ['Warming the local AI model…', 'Embedding your feeling…', 'Comparing 324 film vectors…', 'Ranking hybrid similarity…']
  ...
  return <main className="analysis-screen" aria-live="polite"><div className="analysis-orbit" aria-hidden="true"><i /><i /><i /></div><div className="analysis-copy"><p className="eyebrow"><Sparkles size={13} /> CineMood AI / on this device</p><h1>{error ? 'The local model could not start.' : `${steps[activeStep]}${downloadPercent}`}</h1><p>{error || `Turning “${query}” into a MiniLM vector, then comparing it with the offline CineMood collection.`}</p>{error && <button className="analysis-retry" onClick={onBack}>Back to search</button>}</div><div className="analysis-steps">{steps.map((item, index) => <div className={index < activeStep ? 'is-done' : index === activeStep ? 'is-current' : ''} key={item}><span>{index < activeStep ? <Check size={12} /> : String(index + 1).padStart(2, '0')}</span>{item}</div>)}</div></main>
}
...
function Results({ query, matches, onMovie }) {
  ...
  return <main className="results-page" ref={resultsRef}><section className="results-intro"><p className="eyebrow"><Sparkles size={13} /> Local semantic search complete</p><h1>We found the feeling<br />inside <em>“{query}”</em></h1><p>324 curated TMDb films ranked by 70/30 semantic and keyword similarity—entirely in your browser.</p></section><BestMatch movie={matches[0]} onOpen={onMovie} /><section className="more-matches"><div className="section-heading"><div><p className="eyebrow">Continue the reel</p><h2>More ways into the mood.</h2></div><span>{String(matches.length - 1).padStart(2, '0')} recommendations</span></div><div className="movie-grid">{matches.slice(1).map((movie, index) => <MovieCard key={movie.id} movie={movie} index={index} onOpen={onMovie} />)}</div></section><HowItWorks /></main>
}

function EmptyResults({ onRetry }) {
  return <main className="empty-results"><p className="eyebrow"><Sparkles size={13} /> Search needs a little more signal</p><h1>No strong matches<br /><em>yet.</em></h1><p>Try describing a feeling, scene, relationship, or the aftertaste you want the film to leave behind.</p><button className="timeline-cta" onClick={onRetry}>Try another feeling <ArrowRight size={17} /></button></main>
}
...
function Detail({ movie, onBack }) {
  ...
  return <main className="detail-page" ref={detailRef}><button className="detail-back" onClick={onBack}><ArrowLeft size={17} /> all matches</button><section className="detail-head"><div><p className="eyebrow">Emotional evidence / {movie.match}% {movie.confidenceLabel.toLowerCase()}</p><h1>{movie.title} <span>({movie.year})</span></h1></div><div className="metadata"><span><Clock3 size={14} /> {movie.duration}</span><span><Star size={14} /> {movie.voteAverage?.toFixed?.(1) ?? movie.voteAverage} TMDb</span><span>{movie.mood}</span></div></section><TensionCurve movie={movie} /><section className="detail-context"><div className="detail-poster"><img src={movie.poster} alt={`Poster for ${movie.title}`} /><span className="poster-play"><Play size={17} fill="currentColor" /></span></div><div className="detail-copy"><p>{movie.summary}</p><div className="detail-reason"><span>why it matched</span>{movie.reason}</div><p className="model-note">{movie.isApproximate ? 'This estimated arc is derived from the film’s available review text.' : 'This subtitle-derived arc is mapped from the film’s dialogue over time.'}</p></div></section></main>
}
...
export default function App() {
  ...
  async function beginSearch(nextQuery) {
    ...
    try {
      ...
      const nextView = nextMatches[0]?.shouldShowEmpty ? 'empty' : 'results'
      window.setTimeout(() => { setView(nextView); window.scrollTo(0, 0) }, reduced ? 0 : 360)
    } catch (error) {
      console.error(error)
  ...
  function followCursor(event) { if (!reduced) event.currentTarget.style.setProperty('--cursor-x', `${event.clientX}px`); if (!reduced) event.currentTarget.style.setProperty('--cursor-y', `${event.clientY}px`) }

  return <div className={`app-shell app-shell--${view}`} onPointerMove={followCursor}>{view !== 'intro' && <><div className="ambient-cursor" aria-hidden="true" /><div className="film-grain" aria-hidden="true" /><Header view={view} query={query} onSearch={beginSearch} onHome={goHome} onHowItWorks={showHow} /></>}{view === 'intro' && <Opening onEnter={() => { setView('search'); window.scrollTo(0, 0) }} />}{view === 'search' && <SearchLanding onSearch={beginSearch} />}{view === 'analysis' && <AnalysisScreen {...searchState} query={query} onBack={goHome} />}{view === 'results' && matches.length > 0 && <Results query={query} matches={matches} onMovie={showDetail} />}{view === 'empty' && <EmptyResults onRetry={goHome} />}{view === 'detail' && selected && <Detail movie={selected} onBack={() => setView('results')} />}{view !== 'intro' && <footer><span>CINEMOOD / 2026</span><span>MADE FOR AFTER HOURS</span></footer>}</div>
}
